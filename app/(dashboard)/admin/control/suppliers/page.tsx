...
export default function SuppliersPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const r = await authenticatedFetch("/api/admin/control/suppliers", getAccessToken)
      setRows(r.data ?? [])
    } finally {
      setLoading(false)
    }
  ...
  async function add(e: React.FormEvent) {
    ...
    setAdding(true)
    try {
      const res = await authenticatedFetch("/api/admin/control/suppliers", getAccessToken, {
        method: "POST",
  ...
  async function remove() {
    if (!del) return
    await authenticatedFetch(`/api/admin/control/suppliers/${del.id}`, getAccessToken, { method: "DELETE" })
    setDel(null)
    load()
  }
    ...
    <div className="p-6 space-y-5 max-w-4xl">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-slate-600 text-sm hidden sm:inline">(Fornecedores)</span>
        </div>
        <Button variant="ghost" size="sm" onClick={() => load()} className="text-slate-400">
          <RefreshCw className={`h-4 w-4 ${loading ? "animate-spin" : ""}`} />
        </Button>
      </div>
      <p className="text-slate-500 text-sm">Select a supplier for expenses and contracted services (vendor).</p>

      <Card className="bg-slate-900 border-slate-800">
        <CardHeader>
          <CardTitle className="text-sm text-slate-300">Add supplier</CardTitle>
        </CardHeader>
        <CardContent>
          <form onSubmit={add} className="grid grid-cols-1 sm:grid-cols-4 gap-2">
            ...
            <div className="flex gap-2">
              ...
              />
              <Button type="submit" disabled={adding} className="bg-orange-600 hover:bg-orange-700 text-white shrink-0">
                  ...
                  <TableRow key={r.id} className="border-slate-800">
                    ...
                    <TableCell className="text-right">
                      <Button variant="ghost" size="icon" asChild className="h-8 w-8 text-slate-400">
                        <Link href={`/admin/control/suppliers/${r.id}/edit`}>
