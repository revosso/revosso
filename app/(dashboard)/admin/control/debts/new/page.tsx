...
export default function NewDebtPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setErrors({})
    try {
      const res = await authenticatedFetch("/api/admin/control/debts", getAccessToken, {
        method: "POST",
  ...
  return (
    <div className="p-6 max-w-xl mx-auto space-y-5">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-xl font-bold text-white">Add debt</h1>
      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5 col-span-2">
                ...
                {errors.debtorName && <p className="text-red-400 text-xs">{errors.debtorName[0]}</p>}
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <Input value={form.currency} onChange={(e) => set("currency", e.target.value)}
                  className="bg-slate-800 border-slate-700 text-white" maxLength={10} />
              </div>
              <div className="space-y-1.5">
            ...
            <div className="flex gap-3 pt-2">
              ...
              <Button type="button" variant="ghost" asChild className="text-slate-400">
                <Link href="/admin/control/debts">Cancel</Link>
              </Button>
            </div>
          </form>
        </CardContent>
