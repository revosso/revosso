...
export default function EditClientPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function load() {
      const r = await authenticatedFetch(`/api/admin/control/clients/${id}`, getAccessToken)
      setForm({
        name: r.name ?? "",
        email: r.email ?? "",
        phone: r.phone ?? "",
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true)
    try {
      await authenticatedFetch(`/api/admin/control/clients/${id}`, getAccessToken, {
        method: "PATCH",
      ...
      router.push("/admin/control/clients")
    } finally {
      setSaving(false)
    }
  }
    ...
    <div className="p-6 max-w-lg mx-auto space-y-5">
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="sm" asChild className="text-slate-400">
          <Link href="/admin/control/clients">
