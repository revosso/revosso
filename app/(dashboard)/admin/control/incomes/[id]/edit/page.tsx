...
export default function EditIncomePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function load() {
      const i = await authenticatedFetch(`/api/admin/control/incomes/${id}`, getAccessToken)
      setForm({
      ...
      setClientId(i.clientId != null ? String(i.clientId) : NONE)
      setCategoryId(i.categoryId != null ? String(i.categoryId) : NONE)
      setLoading(false)
    }
    load()
  }, [id, getAccessToken])
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setErrors({})
    try {
      const res = await authenticatedFetch(`/api/admin/control/incomes/${id}`, getAccessToken, {
        method: "PATCH",
  ...
  return (
    <div className="p-6 max-w-xl mx-auto space-y-5">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-xl font-bold text-white">Edit income</h1>
      </div>
