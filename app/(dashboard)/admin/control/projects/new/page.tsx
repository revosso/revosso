...
export default function NewProjectPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setErrors({})
    try {
      const res = await authenticatedFetch("/api/admin/control/projects", getAccessToken, {
        method: "POST",
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto space-y-5">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-xl font-bold text-white">Add project</h1>
      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-1.5">
              ...
              <Input value={form.name} onChange={(e) => set("name", e.target.value)}
                className="bg-slate-800 border-slate-700 text-white" placeholder="Project name" required />
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                {errors.startDate && <p className="text-red-400 text-xs">{errors.startDate[0]}</p>}
              </div>
              <div className="space-y-1.5">
                ...
                <Input type="date" value={form.expectedEndDate} onChange={(e) => set("expectedEndDate", e.target.value)}
                  className="bg-slate-800 border-slate-700 text-white" required />
                {errors.expectedEndDate && <p className="text-red-400 text-xs">{errors.expectedEndDate[0]}</p>}
              </div>
            </div>
