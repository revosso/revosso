...
import {
  ...
  FolderKanban, TrendingUp, TrendingDown, AlertTriangle, Layers, Settings,
} from "lucide-react"
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-center gap-2.5 px-4 h-14 border-b border-slate-800 flex-shrink-0">
        ...
        <div>
          ...
          <span className="text-slate-600 text-xs block -mt-0.5">{t.brandAdmin}</span>
        </div>
      </div>
      ...
      <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
        ...
        {/* Control section */}
        <p className="px-3 pt-3 pb-1 text-xs font-semibold text-slate-600 uppercase tracking-wider">{t.sectionControl}</p>
        {controlNav.map(({ href, icon: Icon, label }) => (
          <Link
            key={href}
            href={href}
            onClick={onLinkClick}
    ...
    <div className="flex min-h-screen bg-slate-950">
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 z-30 h-14 flex items-center justify-between px-4 border-b border-slate-800 bg-slate-950/90 backdrop-blur-xl">
        <div className="flex items-center gap-2">
          ...
          <span className="text-slate-400 text-sm">{t.brandAdmin}</span>
        </div>
        <Button
          variant="ghost"
          size="icon"
